...
void Wind(const tN2kMsg &N2kMsg){
  ...
  if (ParseN2kPGN130306(N2kMsg, Instance, WindSpeed, WindAngle, WindReference)){
    PrintLabelValWithConversionCheckUnDef("Wind: ", Instance, 0, true);
    PrintLabelValWithConversionCheckUnDef("  speed: ", WindSpeed, &msToKnots, true);
    PrintLabelValWithConversionCheckUnDef("  Angle: ", WindAngle, &RadToDeg, true);
  } else {
    OutputStream->print("Failed to parse PGN: "); OutputStream->println(N2kMsg.PGN);
  }
